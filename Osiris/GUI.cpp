...
void GUI::renderVisualsWindow(bool contentOnly) noexcept
{
    ...
    if (!config->visuals.fullBright) {
        ...
        ImGui::PopID();
        if (config->visuals.brightness == 1.0f) { config->visuals.fullBright = 1; };
    }
    else {
        ImGui::PushID(6);
        ImGui::SliderFloat("", &config->visuals.brightness, 0.0f, 1.0f, "Full Bright Enabled");
        ImGui::PopID();
        if (config->visuals.brightness <= 0.999f) { config->visuals.fullBright = 0; };
    };
    ImGui::PopItemWidth();
