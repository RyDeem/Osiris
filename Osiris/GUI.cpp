...
void GUI::renderChamsWindow(bool contentOnly) noexcept
{
    ...
    ImGui::SliderFloat("", &chams.alpha, 0.0f, 1.0f, "Alpha %.2f");
    ImGui::SameLine();
...
void GUI::renderVisualsWindow(bool contentOnly) noexcept
{
    if (!contentOnly) {
        if (!window.visuals)
            return;
        ImGui::SetNextWindowSize({ 660.0f, 0.0f });
        ImGui::Begin("Visuals", &window.visuals, windowFlags);
    }
    ImGui::Columns(2, nullptr, false);
    ImGui::SetColumnOffset(1, 330.0f);
    ...
    ImGui::SameLine();
    ImGui::Checkbox("Deagle spin", &config->visuals.deagleSpinner);
    ...
    ImGui::Text("Custom Viewmodel XYZ");
        ...
        ImGui::SameLine();
        if (!viewModelType) {ImGui::Checkbox("Swap Gun Side", &config->visuals.viewmodel_xyz_clrightguns);};
        if (viewModelType) {ImGui::Checkbox("Swap Knife Side", &config->visuals.viewmodel_xyz_clrightknife);};
    ...
    ImGui::Combo("Hit marker", &config->visuals.hitMarker, "None\0Default (Cross)\0Crosshair Color (Cross)\0");
...
void GUI::renderMiscWindow(bool contentOnly) noexcept
{
    if (!contentOnly) {
        if (!window.misc)
            return;
        ImGui::SetNextWindowSize({ 570.0f, 0.0f });
    ...
    ImGui::SameLine();
    hotkey(config->misc.prepareRevolverKey);//          0     1        2        3      4      5
